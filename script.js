...
while (matches < match_iteration) {
    ...
    let grid_value = 0 // 511 means 9 zone filled
    while (grid_value < 511) {
        ...
        if (x_phase) {
            ...
            // THE PLAYER
            const matchInput = find_best_move(tris_grid, deep_mind, matches)//  getRandomInt(9) // il training sarà sempre fatto con numeri random
...
console.log('\n')
console.log(`è il momemto di giocare..`)
let one_more = 'y'
...
while(one_more =='y') {
    ...
    console.log(winner_found, 'win')
    one_more = promptSync()('vuoi fare un\'altra partita? (y) ') || 'y'
}